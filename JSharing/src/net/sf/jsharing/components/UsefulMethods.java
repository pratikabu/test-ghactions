...
public class UsefulMethods {
	...
	public static void loadData() {
		...
		String date = cal.get(Calendar.DATE) + "-" + cal.get(Calendar.MONDAY)
				+ "-" + cal.get(Calendar.YEAR);
		try {
			log = new FileLog("JSharing Logs", FileModule.LOG_FOLDER
					+ File.separatorChar + date + ".xml", true);
		} catch (IOException e) {
			e.printStackTrace();
		}
	...
	public static String getLastSavedLocation() {
		...
		if (value == null)
			value = FileSystemView.getFileSystemView().getDefaultDirectory()
					.getAbsolutePath();
	...
	private static void loadProperties() {
		try {
			FileInputStream fis = new FileInputStream(new File(
					FileModule.PROPERTIES_LOCATION));
			props.load(fis);
			fis.close();
		} catch (IOException e) {
		}
	...
	public static void saveProperties() {
		try {
			props.store(new FileOutputStream(new File(
					FileModule.PROPERTIES_LOCATION)),
					"The properties have been modified at: " + new Date());
		} catch (IOException e) {
	...
	public static void saveSavedIPs() {
		try {
			...
			for (Entry<String, SavedIPInfo> entry : savedIPs.entrySet()) {
				...
				pw.print(sip.getIp() + "," + sip.getName() + ","
						+ sip.getPort());
			}
			pw.close();
		} catch (IOException e) {
	...
	public static void browse(String urlStr) {
		if (Desktop.isDesktopSupported()) {
			...
			JOptionPane.showMessageDialog(MainWindow.mw,
					"Cannot open the URL. Kindly open this url\n"
							+ "from your browser:\n" + urlStr);
		}
	}
